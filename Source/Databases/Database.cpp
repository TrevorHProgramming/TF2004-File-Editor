...
void DefinitionFile::readDictionary(QByteArray splitLine, int sectionIndex, QString instanceName){
    ...
    while(tempRead.currentPosition < tempRead.dataBytes.size()){
        ...
        tempRead.hexValue(&itemDetails.type, typeLength);
        ...
        QString activeType;
        tempRead.hexValue(&activeType, activeLength);
        if(activeType == "False"){
        ...
        tempRead.hexValue(&itemDetails.name, nameLength);
    ...
    */
    ...
    while (parent->fileData.currentPosition != -1){
        ...
        if (majorName == "IncludedFiles") {
            ...
            QString realPath;
            tempRead.hexValue(&realPath, pathLength);
        ...
        while(tempRead.currentPosition < tempRead.dataBytes.length()){
            ...
            tempRead.hexValue(&byteItem, sectionLength -4);
...
/*void DefinitionFile::editDatabaseItem(QModelIndex item, int itemIndex){
    ...
    for(int i = 0; i < parent->definitions.size(); i++){
        if(item.parent().parent().data().toString() == parent->definitions[i].fileName){
            newValue = parent->definitions[i].editItem(item.parent().data().toString(), itemIndex);
        }
    }
    for(int i = 0; i < parent->databases.size(); i++){
        if(item.parent().parent().data().toString() == parent->databases[i].fileName){
            newValue = parent->databases[i].editItem(item.parent().siblingAtColumn(1).data().toInt(), itemIndex);
        }
    }

    if(newValue[0] == "SINGLEVALUE"){
        parent->testModel->setData(item.siblingAtColumn(2), newValue[1]);
    } else if (newValue[0] == "ENUM"){
        parent->testModel->setData(item.siblingAtColumn(2), newValue[1]);
        newValue.remove(0,2);
        parent->testModel->setData(item.siblingAtColumn(3), newValue.join(','));
    } else if (newValue[0] == "SETDEFAULT"){
        parent->testModel->setData(item.siblingAtColumn(4), true);
    } else {
        parent->testModel->setData(item.siblingAtColumn(3), newValue.join(','));
    }
...
void DefinitionFile::removeDatabaseClass(QModelIndex item){
    for(int i = 0; i < parent->definitions.size(); i++){
        if(item.parent().data().toString() == parent->definitions[i].fileName){
           parent->definitions[i].removeClass(item.row());
        }
    }
    for(int i = 0; i < parent->databases.size(); i++){
        if(item.parent().data().toString() == parent->databases[0].fileName){
            parent->databases[i].removeInstance(item.row());
        }
    }
    parent->testModel->removeRows(item.row(), 1, item.parent());
}

void DefinitionFile::removeDatabaseItem(QModelIndex item, int itemIndex){
    for(int i = 0; i < parent->definitions.size(); i++){
        if(item.parent().parent().data().toString() == parent->definitions[i].fileName){
            parent->definitions[i].removeItem(item.parent().data().toString(), itemIndex);
        }
    }
    for(int i = 0; i < parent->databases.size(); i++){
        if(item.parent().parent().data().toString() == parent->databases[0].fileName){
            parent->databases[i].removeItem(item.parent().siblingAtColumn(1).data().toInt(), itemIndex);
        }
    }
